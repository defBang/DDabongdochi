<template>
  <div>
    <div class="pagebody" v-if="!isError">
      <div class="postboard">자유 게시판</div>
      <div class="modeButton">
        <b-button-group>
          <b-button
            variant="outline-dark"
            @click="this.$router.push({ name: 'free' })"
          >
            전체글
          </b-button>
          <b-button
            variant="danger"
            @click="
              this.$router.push({ name: 'free', query: { mode: 'best' } })
            "
          >
            추천글
          </b-button>
      ...
      <div
        v-if="$route.name == 'free' || $route.name == 'home'"
        class="postDetail"
      >
        <div class="postInfo">
          <b-row>
        ...
        <div v-for="post in posts" :key="post.free_post_id" class="Postlist">
...
<script>
import Postlist from "../components/ListFree.vue";
export default {
  components: {
    Postlist,
  },
  ...
  methods: {
    pageTo(index) {
      ...
      this.$router.push({ name: "free", query: qString });
    },
    doSearch() {
      var qString = {};
      if (this.word.search != "") {
    ...
    posting() {
      if (this.$store.state.user.isSignIn) {
        this.$router.push({ name: "free_write" });
      } else {
