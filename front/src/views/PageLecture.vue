<template>
  <div>
    <div class="pagebody" v-if="!isError">
      <div class="postboard">강의 정보</div>
      <div class="modeButton">
        <b-button-group>
          <b-button
            variant="outline-dark"
            @click="this.$router.push({ name: 'lecture' })"
          >
            전체글
          </b-button>
          <b-button
            variant="danger"
            @click="
              this.$router.push({ name: 'lecture', query: { mode: 'best' } })
            "
          >
            추천글
          </b-button>
      ...
      <div
        v-if="$route.name == 'lecture' || $route.name == 'home'"
        class="postDetail"
      >
        <div class="postInfo">
          <b-row>
            ...
            <b-col class="postLecturerInfo" sm="auto"> 강의자 </b-col>
            <b-col class="postPlatformInfo" sm="auto"> 플랫폼 </b-col>
        ...
        <div v-for="post in posts" :key="post.lecture_post_id" class="Postlist">
...
<script>
import Postlist from "../components/ListLecture.vue";
export default {
  components: {
    Postlist,
  },
  ...
  mounted() {
    ...
    if (this.$route.query.searchoption !== undefined) {
      ...
      switch (this.$route.query.searchoption) {
        ...
        case "lecturer":
          axiosQuery = {
            lecturer: this.$route.query.word,
            p: this.$route.query.p,
          };
          break;
        case "tag":
  ...
  methods: {
    pageTo(index) {
      ...
      this.$router.push({ name: "lecture", query: qString });
    },
    doSearch() {
      var qString = {};
      if (this.word.search != "") {
    ...
    posting() {
      if (this.$store.state.user.isSignIn) {
        this.$router.push({ name: "lecture_write" });
      } else {
