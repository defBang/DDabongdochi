<template>
  <div>
    <div class="pagebody" v-if="!isError">
      <div class="postboard">유튜브 채널</div>
      <div class="modeButton">
        <b-button-group>
          <b-button
            variant="outline-dark"
            @click="this.$router.push({ name: 'channel' })"
          >
            전체글
          </b-button>
          <b-button
            variant="danger"
            @click="
              this.$router.push({ name: 'channel', query: { mode: 'best' } })
            "
          >
            추천글
          </b-button>
          <b-button variant="outline-dark" @click="posting">
            채널 등록
          </b-button>
        </b-button-group>
      </div>
      <div
        v-if="$route.name == 'channel' || $route.name == 'home'"
        class="postDetail"
      >
        <div class="postInfo">
          <b-row>
            <b-col sm="auto" class="postNoInfo"> 번호 </b-col>
            <b-col class="postTitleInfo"> 제목 </b-col>
            <b-col sm="auto" class="postTagInfo"> 태그 </b-col>
            <b-col sm="auto" class="postDateInfo"> 작성일 </b-col>
            <b-col sm="auto" class="postRateInfo"> 추천수 </b-col>
          </b-row>
        </div>
        <div v-for="post in posts" :key="post.channel_post_id" class="Postlist">
...
<script>
import Postlist from "../components/ListChannel.vue";
export default {
  components: {
    Postlist,
  },
  ...
  methods: {
    pageTo(index) {
      ...
      this.$router.push({ name: "channel", query: qString });
    },
    doSearch() {
      var qString = {};
      if (this.word.search != "") {
    ...
    posting() {
      if (this.$store.state.user.isSignIn) {
        this.$router.push({ name: "channel_write" });
      } else {
